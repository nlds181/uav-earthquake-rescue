...
# ==================== 目标函数（简化，确保返回float） ====================
def build_objective(n_uav, users, objective_type="coverage"):
    def obj(x):
        cost = 0.0
        u_pos = users.get()
        ...
        for i in range(n_uav):
            for j in range(i+1, n_uav):
                ...
                if dist < 45:
                    cost += (45 - dist) * 12
        return float(cost)  # 确保返回 Python float
    return obj
...
def main():
    ...
    <div style='text-align: center; padding: 15px; background: linear-gradient(135deg, #1e3c72, #2a5298); border-radius: 15px; margin-bottom: 20px'>
        ...
        <p style='color: #aaf; font-size: 14px'>✅ 多无人机稳定 | 虚线轨迹 | 大范围快速飞行</p>
    </div>
    ...
    with st.sidebar:
        ...
        max_iter = st.slider("优化迭代次数", 30, 150, 80)
    ...
    if run:
        with st.spinner("🔄 优化引擎运行中... 无人机将快速向用户区域移动"):
            try:
                users = Users(n_users)
                obj_fn = build_objective(n_uav, users, obj_type)
        ...
        st.markdown("""
        ### 📖 作品特色
        - **多无人机稳定**：简化优化器，确保无数组错误。
        - **虚线轨迹**：无人机历史轨迹用虚线显示，当前点为实心圆。
        - **大范围快速运动**：初始半径400m，学习率0.12，运动更快。
        - **立体山丘地形**：范围-500~500m，多山峰，带等高线。
        - **动态三维飞行**：无人机水平移动同时高度随位置变化，形成真实三维轨迹。
        - **地形用户不消失**：每帧重建，稳定显示。
        """)
